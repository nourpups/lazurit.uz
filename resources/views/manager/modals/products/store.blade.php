    ...
    <div class="modal-content">
      <div class="block-rounded mb-0 block shadow-none">
        ...
        <div class="block-content fs-sm pb-3">
          <form action="{{ route('product.store') }}" enctype="multipart/form-data" method="POST" id="store">
            @csrf
            @foreach (config('translatable.locales') as $locale)
              <div class="form-floating mb-2">
                <input type="text" class="form-control" name="{{ $locale }}[name]"
                  value="{{ old($locale . '.name') }}" placeholder="Product Name [{{ $locale }}]">
                <label class="form-label">Name [{{ $locale }}]</label>
              </div>
              <div class="form-floating mb-2">
                <textarea class="form-control" name="{{ $locale }}[description]" style="height: 200px"
                  placeholder="Product Description [{{ $locale }}]">{{ old($locale . '.description') }}</textarea>
